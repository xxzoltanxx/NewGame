...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    public WorldPlayerActionHandler handler;
   ...
   public void addExecAction(Ability ability)
    {
        handler.AddAbilityAndTriggerCooldown(ability);
    }
    ...
    void Update() {
        ...
        if (updateProgress)
        {
            Image progressBar = transform.GetChild(2).GetChild(0).GetComponent<Image>();
            progressBar.fillAmount = 1.0f - handler.playerAbilities.cooldown / handler.playerAbilities.cooldownTime;
            if (handler.playerAbilities.cooldown == 0)
            {
                updateProgress = false;
                progressBar.fillAmount = 0.0f;
                for (int i = 0; i < elementCount; ++i)
                {
                    if (elements[i].hasSkill())
                    {
                        elements[i].setSkillActive();
                    }
                }
            }
        }
        ...
        if (!useGamepad)
            rawAngle = Mathf.Atan2(Camera.main.ScreenToWorldPoint(Input.mousePosition).y - rt.position.y, Camera.main.ScreenToWorldPoint(Input.mousePosition).x - rt.position.x) * Mathf.Rad2Deg;
        else
