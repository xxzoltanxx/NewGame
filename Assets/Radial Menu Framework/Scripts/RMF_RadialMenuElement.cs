...
public class RMF_RadialMenuElement : MonoBehaviour {
    ...
    public void setAbility(Sprite tex, bool enabled, Ability ability)
    {
        this.ability = ability;
        transform.GetChild(0).GetComponent<Button>().onClick.RemoveAllListeners();
        transform.GetChild(0).GetComponent<Button>().onClick.AddListener(triggerIt);
        if (tex != null)
        {
            transform.GetChild(0).GetChild(0).GetComponent<Image>().sprite = tex;
        }
        else
        {
    ...
    void triggerIt()
    {
        parentRM.addExecAction(ability);
        parentRM.close();
    }
