...
public class AffinityBar : MonoBehaviour
{
    ...
    {
        transform.GetChild(2).gameObject.GetComponent<Text>().text = name;
    }
    ...
    {
        transform.GetChild(1).GetChild(1).gameObject.GetComponent<Text>().text = wagons.ToString();
    }
    ...
    {
        scale = scale / 3;
        transform.GetChild(2).gameObject.GetComponent<RectTransform>().localScale = new Vector3(scale, scale, scale);
